...
async def get_complete_summary(
    ...
    """Полная сводка за период (все данные одним запросом)"""
    try:
        user_id = current_user["id"]
        
        # Определяем количество дней по периоду
        days_map = {
            "week": 7,
            "month": 30,
            "quarter": 90,
        ...
        overview = await db.statistics.get_overview_stats(user_id, days)
        chart_data = await db.statistics.get_daily_activity_chart(user_id, days)
        platform_dist = await db.statistics.get_platform_distribution(user_id, days)
        top_apps = await db.statistics.get_top_apps(user_id, 5, days)  # Только топ-5
        ...
        return {
            ...
            "overview": overview,
            "chart_data": chart_data,
            "platform_distribution": platform_dist["distribution"],
            "top_apps": top_apps,
