...
from sqlalchemy import (
    ...
    Float, Text, JSON, ForeignKey, UniqueConstraint,
    Index, text, Enum, LargeBinary
)
...
class Device(Base):
    ...
    platform_version: Mapped[Optional[str]] = mapped_column(
        String(100),
        nullable=True,
        comment="Версия платформы/ОС"
    )
    client_version: Mapped[Optional[str]] = mapped_column(
        String(50),
        nullable=True,
        comment="Версия клиента ActivityWatch Sync"
    )
    is_active: Mapped[bool] = mapped_column(
        Boolean,
        default=True,
        nullable=False,
        comment="Активно ли устройство"
    )
...
class ApiToken(Base):
    ...
    expires_at: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        comment="Дата истечения срока действия"
    )
...
class SyncSession(Base):
    ...
    end_time: Mapped[Optional[datetime]] = mapped_column(
        DateTime(timezone=True),
        nullable=True,
        comment="Время окончания синхронизации"
    )
    events_count: Mapped[int] = mapped_column(
        Integer,
        default=0,
        nullable=False,
        comment="Количество обработанных событий"
    )
...
class ActivityEvent(Base):
    ...
    event_id: Mapped[str] = mapped_column(
        String(255),
        nullable=False,
        comment="Уникальный ID события из ActivityWatch"
    )
    ...
    duration_seconds: Mapped[float] = mapped_column(
        Float,
        nullable=False,
        comment="Длительность события в секундах"
    )
    ...
    window_title: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="Заголовок окна"
    )
    url: Mapped[Optional[str]] = mapped_column(
        Text,
        nullable=True,
        comment="URL (для браузеров)"
    )
    category: Mapped[Optional[str]] = mapped_column(
        String(100),
        nullable=True,
        comment="Категория активности"
    )
...
class DailySummary(Base):
    ...
    total_time: Mapped[float] = mapped_column(
        ...
        default=0.0,
        nullable=False,
        comment="Общее время активности (секунды)"
    )
